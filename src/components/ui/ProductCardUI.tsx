        ...
        <Card className="glass-effect border-0 overflow-hidden h-full">
          <CardContent className="p-4">
            <Link to={`/products/${logic.product.slug}`} className="block">
              ...
              <h3 className="text-foreground font-bold text-sm mb-1 line-clamp-2">
                {logic.product.title}
              </h3>
            ...
            {logic.hasVariants && logic.options && (
              <div className="mb-3 space-y-2">
                {logic.options.map((opt) => (
                  <div key={opt.id}>
                    <div className="text-xs font-bold text-foreground mb-1">{opt.name}</div>
                      ...
                      {opt.values.filter(val => logic.isOptionValueAvailable(opt.name, val)).map((val) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() => logic.handleOptionChange(opt.name, val)}
                            className={`border rounded-lg px-3 py-1 text-xs font-bold transition-all duration-200 ${
                              isSelected 
                                ? 'y2k-gradient y2k-border' 
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                <span className="y2k-text-gradient font-black text-lg">
              ...
              <Button
                ...
                className="y2k-gradient font-bold hover:scale-105 transition-transform duration-200 disabled:opacity-50"
              >
